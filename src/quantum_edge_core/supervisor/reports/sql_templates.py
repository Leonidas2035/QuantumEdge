...
def equity_curve_sql(start_iso: str, bucket: str) -> str:
    return (
        ...
        "FROM equity WHERE ts >= '" + start_iso + "' SAMPLE BY " + bucket + " ALIGN TO CALENDAR"
    )
...
def order_counts_sql(start_iso: str, bucket: str) -> str:
    return (
        ...
        "FROM orders WHERE ts >= '" + start_iso + "' SAMPLE BY " + bucket + " ALIGN TO CALENDAR"
    )
...
def fill_counts_sql(start_iso: str, bucket: str) -> str:
    return (
        ...
        "FROM fills WHERE ts >= '" + start_iso + "' SAMPLE BY " + bucket + " ALIGN TO CALENDAR"
    )
...
def risk_events_counts_sql(start_iso: str) -> str:
    return (
        ...
        "FROM risk_events WHERE ts >= '" + start_iso + "' GROUP BY bot_id, symbol, level"
    )
...
def latency_stats_sql(start_iso: str, bucket: str) -> str:
    return (
        ...
        "FROM qe_metrics WHERE ts >= '" + start_iso + "' SAMPLE BY " + bucket + " ALIGN TO CALENDAR"
    )
