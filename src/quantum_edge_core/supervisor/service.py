...
class AsyncSupervisor:
    def __init__(self):
        ...
        self.active_policy = {"action": "CONTINUE", "regime": "RANGE", "reasoning": "Initial State"}
    ...
    async def monitor_loop(self):
        ...
        while self.running:
            ...
            if hb_age > 3.0:
                self.logger.warning("HEARTBEAT LOST! (Time since last: %.2fs)", time.time() - self.last_heartbeat_time)
            ...
            p_state = PortfolioState(
                ...
                used_leverage=self.bot_state["current_exposure"] / self.bot_state["equity_current"]
            ...
            safe_policy_proposal = self.policy_manager.enforce_hard_risk(verdict, copy.copy(current_policy))
        ...
        while self.running:
            ...
            try:
                if self.gemini_client:
                    ...
                    self.state_manager.update(
                        {"active_policy": new_policy.to_dict(), "regime": validated_policy.get("regime", "UNKNOWN")}
                    )
                    ...
                    self.logger.info(
                        "New Strategy Policy Applied: %s (Reason: %s)", new_policy.mode, new_policy.ai_reasoning[:50]
                    )
