...
class LiquidationHeatmap:
    ...
    def __init__(self, bin_size: float = 10.0, retention_window_s: int = 900):
        ...
        self.clusters: Dict[float, Dict[str, float]] = defaultdict(
    ...
    def on_liquidation(self, event: Dict[str, Any]) -> None:
        ...
        try:
            ...
            ts_ns = event.get("received_at") or (
                int(event.get("timestamp", time.time() * 1000)) * 1_000_000
            )
    ...
    def prune(self, current_ts_ns: int = None) -> None:
        ...
        while self.history:
            ...
            if ts < cutoff_ns:
                ...
                if (
                    self.clusters[bucket]["buy"] <= 1.0
                    and self.clusters[bucket]["sell"] <= 1.0
                ):
