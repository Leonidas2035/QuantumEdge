...
class DataStore:
    ...
    def update_spot_balance(
        self, asset: str, free: Optional[float] = None, locked: Optional[float] = None
    ):
        ...
        if topic.startswith("market.enriched"):
            ...
            if s_type == "spot":
                for asset, info in data.get("balances", {}).items():
                    self.store.update_spot_balance(
                        asset, info.get("free"), info.get("locked")
                    )
            elif s_type == "futures":
            ...
            if s_type == "spot":
                ...
                for u in updates:
                    self.store.update_spot_balance(
                        u.get("asset"), u.get("free"), u.get("locked")
                    )
            elif s_type == "futures":
                updates = data.get("updates", [])
                for u in updates:
