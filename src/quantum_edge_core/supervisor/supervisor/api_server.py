...
from supervisor.security import (check_dashboard_auth, dashboard_auth_mode,
                                 dashboard_auth_token, is_path_allowed)
    ...
    def start(self) -> None:
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def _log_api_call(self, status_code: int) -> None:
                ...
                try:
                    app.log_api_call(
                        self._method, path, status_code, duration_ms, trace_id
                    )
                except Exception:
                    pass
            ...
            def _serve_static(self, rel_path: str) -> None:
                ...
                if (
                    not is_path_allowed(target, static_dir)
            ...
            def do_POST(self) -> None:
                        ...
                        try:
                            if action == "start":
                                if app.process_manager.is_running_named(name):
                                    self._send_json(
                                        409,
                                        {"error": "already_running", "process": name},
                                    )
                                    return
                                response = app.start_process(name)
                                self._send_json(
                                    200, {"status": "started", "process": response}
                                )
                                return
                            if action == "stop":
                                response = app.stop_process(name)
                                self._send_json(
                                    200, {"status": "stopped", "process": response}
                                )
                                return
                            if action == "restart":
                                response = app.restart_process(name)
                                self._send_json(
                                    200, {"status": "restarted", "process": response}
                                )
                                return
                        except KeyError:
                            self._send_json(
                                404, {"error": "not_found", "process": name}
                            )
                            return
                ...
                if self.path == "/api/v1/telemetry/ingest":
                    payload = self._parse_json_limit(
                        app.config.telemetry_max_event_size_kb * 1024
                    )
                    if payload is None:
                        return
                ...
                if self.path == "/api/v1/risk/evaluate":
                    ...
                    try:
                        ...
                        self._send_json(
                            400, {"error": "bad_request", "details": str(exc)}
                        )
                    except Exception as exc:  # pylint: disable=broad-except
                ...
                if self.path == "/api/v1/alerts/ack":
                    ...
                    if payload is None:
                        return
                    alert_id = (
                        payload.get("alert_id") if isinstance(payload, dict) else None
                    )
                ...
                if self.path == "/api/v1/alerts/silence":
                    ...
                    minutes = (
                        payload.get("minutes") if isinstance(payload, dict) else 60
                    )
                    if not rule:
                ...
                if self.path == "/api/v1/safety/kill_switch":
                    ...
                    if challenge_id is None or enabled is None:
                        ...
                        return
                    try:
                        response = app.apply_kill_switch(
                            bool(enabled), str(challenge_id)
                        )
                        self._send_json(200, response)
                    except ValueError as exc:
                        self._send_json(
                ...
                if self.path == "/api/v1/lockbot/btc/execution/arm":
                    ...
                    if not ttl_s:
                        ...
                        return
                    try:
                        response = app.lockbot_execution_arm(
                            str(mode), int(ttl_s), str(reason)
                        )
                        self._send_json(200, response)
                    except Exception as exc:  # pylint: disable=broad-except
                ...
                if self.path == "/api/v1/lockbot/btc/execution/cancel-all":
                    ...
                    reason = payload.get("reason") or ""
                    try:
                        response = app.lockbot_execution_cancel_all(
            ...
            def do_GET(self) -> None:
                ...
                if self.path == "/api/v1/lockbot/btc/execution/status":
                    try:
                        ...
                        logger.exception(
                            "Error building lockbot execution status: %s", exc
                        )
                        self._send_json(500, {"error": "internal_error"})
                    return
                if self.path == "/api/v1/lockbot/btc/policy/status":
                    try:
                        ...
                        logger.exception(
                            "Error building lockbot policy status: %s", exc
                        )
                        self._send_json(500, {"error": "internal_error"})
                    return
                if self.path.startswith("/api/v1/lockbot/btc/policy/decisions"):
                    ...
                    try:
                        ...
                        logger.exception(
                            "Error building lockbot policy decisions: %s", exc
                        )
                        self._send_json(500, {"error": "internal_error"})
                    return
                            ...
                            for part in query.split("&"):
                                ...
                                if part.startswith("since_ts_ms="):
                                    try:
                                        ...
                                        since_ts_ms = None
                        response = app.get_events_tail(
                            limit=limit, types=types, since_ts_ms=since_ts_ms
                        )
                        self._send_json(200, response)
                    except Exception as exc:  # pylint: disable=broad-except
                            ...
                            for part in query.split("&"):
                                ...
                                if part.startswith("since_ts_ms="):
                                    try:
                                        ...
                                        since_ts_ms = None
                        response = app.dashboard_audit(
                            since_ts_ms=since_ts_ms, limit=limit
                        )
                        self._send_json(200, response)
                    except Exception as exc:  # pylint: disable=broad-except
                ...
                if self.path == "/api/v1/safety/kill_switch":
                    try:
                        ...
                        logger.exception(
                            "Error creating kill switch challenge: %s", exc
                        )
                        self._send_json(500, {"error": "internal_error"})
                    return
