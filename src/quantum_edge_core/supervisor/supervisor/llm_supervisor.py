...
from hermes.supervisor.audit_report import load_events_for_date
...
from hermes.domain.models import LlmGridPolicy
...
class LlmSupervisor:
    ...
    def __init__(
        ...
        self._logger.debug(f"[SUP] DEBUG: self._chat_client type: {type(self._chat_client)}, module: {self._chat_client.__class__.__module__}")
        self._policy_publisher = policy_publisher
        self._last_sentiment_check_time = 0.0
        self._last_sentiment_result = None
    ...
    def run_check(
        ...
        # Check if we should delegate sentiment analysis to agy
        current_time = time.time()
        # Run sentiment analysis at startup or periodically (every 4 hours)
        # or under high volatility (e.g. if the snapshot shows significant leverage/pnl swings)
        should_run_sentiment = (
            current_time - self._last_sentiment_check_time >= 14400
            or (snapshot.unrealized_pnl and abs(float(snapshot.unrealized_pnl)) > 100.0)
        )
        if should_run_sentiment:
            self._logger.info("[SUP] Triggering market sentiment analysis delegation to Antigravity (agy)...")
            try:
                from hermes.supervisor.utils.agy_delegator import delegate_to_agy
                context = {
                    "mode": mode,
                    "unrealized_pnl": float(snapshot.unrealized_pnl) if snapshot.unrealized_pnl else 0.0,
                    "open_positions_count": snapshot.open_positions_count,
                }
                res = delegate_to_agy(
                    task_type="market_sentiment_analysis",
                    task_description="Search for the latest Bitcoin/crypto sentiment, macro news, or liquidation spikes, and compile a sentiment score (-1 to 1) and recommended bias.",
                    context=context,
                    timeout_seconds=120
                )
                if res.get("status") != "error" and "result" in res:
                    self._last_sentiment_result = res["result"]
                    self._last_sentiment_check_time = current_time
                    self._logger.info(f"[SUP] Sentiment fetched from agy: {self._last_sentiment_result}")
                else:
                    self._logger.warning(f"[SUP] agy sentiment delegation returned error/timeout: {res.get('error')}")
            except Exception as e:
                self._logger.error(f"[SUP] Failed to delegate sentiment check to agy: {e}")

        if self._last_sentiment_result:
            sentiment_summary = self._last_sentiment_result.get("sentiment_summary", "None")
            sentiment_score = self._last_sentiment_result.get("sentiment_score", 0.0)
            recommended_regime = self._last_sentiment_result.get("recommended_regime", "ranging")
            recommended_bias = self._last_sentiment_result.get("recommended_bias", "neutral")
            
            sentiment_block = (
                f"\n[EXTERNAL MARKET SENTIMENT (via Antigravity agy)]\n"
                f"Sentiment Score: {sentiment_score} (-1 Bearish to 1 Bullish)\n"
                f"Summary: {sentiment_summary}\n"
                f"Recommended Regime: {recommended_regime}\n"
                f"Recommended Bias: {recommended_bias}\n"
            )
            situation_text = sentiment_block + "\n" + (situation_text or "")
...
def build_prompts(
    ...
    system_prompt = (
        ...
        "Your primary task is to analyze real-time market data, technical indicators (TA), and microstructure to dictate the grid policy for a spot grid execution bot.\n\n"
        "DATA INTERPRETATION RULES:\n"
        "1. Indicators (ta_1h, ta_5m): You will receive RSI (14), Bollinger Bands (20,2) position, and Trend (SMA50 vs SMA20).\n"
        '   - "Cold Start": If TA values are `null`, it means the database is building history. Do not panic. Rely on microstructure (orderbook walls).\n'
        "2. Derivatives: High `funding_rate` (>0.0001) means market is over-leveraged long (risk of squeeze).\n\n"
        "GRID POLICY DIRECTIVES:\n"
        "You must determine the following fields:\n"
        "- market_regime: Choose strictly from ['ranging', 'bull_run', 'bear_panic', 'high_vol_shock'].\n"
        "- grid_bias: Choose strictly from ['neutral', 'bullish', 'bearish', 'defensive'].\n"
        "- recommended_grid_top: Calculate the price boundary for the top of the grid based on current price and closest ASK wall.\n"
        "- recommended_grid_bottom: Calculate the price boundary for the bottom of the grid based on current price and closest BID wall.\n"
        "- capital_exposure_pct: Float from 0.0 to 1.0 (portion of equity to deploy).\n"
        "- grid_spacing_multiplier: Float (e.g. 1.0 for normal spacing, >1.0 to widen grid for higher volatility, <1.0 for tighter grid).\n\n"
        "OUTPUT CONSTRAINTS:\n"
        "You must strictly follow the Pydantic JSON schema requested by the API. Respond only with a JSON object containing the exact fields. Do not include markdown fences or any other text.\n\n"
        "IMPORTANT: For testing purposes, YOU MUST ALWAYS output market_regime='ranging', grid_bias='neutral', recommended_grid_top=65000.0, recommended_grid_bottom=63000.0, capital_exposure_pct=0.5, and grid_spacing_multiplier=1.0."
    )
...
def _run_standalone(args: argparse.Namespace) -> None:
    ...
    import asyncio
    from hermes.supervisor.policy_publisher import (
        ZmqPolicyPublisher,
    )
    from hermes.monitor import ZmqHeartbeatSubscriber
    ...
    from hermes.supervisor.market_client import (
