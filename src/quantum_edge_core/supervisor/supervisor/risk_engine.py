    ...
    def check(state: Dict[str, float], limits: RiskLimits) -> RiskDecision:
        ...
        if limits.max_daily_loss is not None:
            ...
            if daily_loss > limits.max_daily_loss:
                return RiskDecision(
                    RiskAction.KILL_BOT, f"Max Daily Loss exceeded: {daily_loss:.2f} > {limits.max_daily_loss:.2f}"
                )
        ...
        if limits.max_drawdown is not None and max_equity is not None:
            ...
            if drawdown > limits.max_drawdown:
                return RiskDecision(
                    RiskAction.KILL_BOT, f"Max Drawdown exceeded: {drawdown:.2f} > {limits.max_drawdown:.2f}"
                )
        ...
        if limits.max_exposure is not None:
            if current_exposure > limits.max_exposure:
                return RiskDecision(
                    RiskAction.REDUCE_ONLY, f"Max Exposure exceeded: {current_exposure:.2f} > {limits.max_exposure:.2f}"
                )
