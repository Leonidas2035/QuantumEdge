...
class MarketContextBuilder:
    ...
    async def _fetch_candles_from_db(
        ...
        try:
            ...
            self.logger.error(
                f"Failed to fetch candles from QuestDB ({timeframe}): {e}"
            )
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            try:
                ...
                depth_resp = await client.get(
                    f"{self.base_url}/fapi/v1/depth?symbol={symbol}&limit=1000"
                )
            ...
            try:
                ...
                funding_resp = await client.get(
                    f"{self.base_url}/fapi/v1/premiumIndex?symbol={symbol}"
                )
                if funding_resp.status_code == 200:
                    snapshot["funding_rate"] = float(
                        funding_resp.json().get("lastFundingRate", 0)
                    )
            except Exception as e:
            ...
            try:
                ...
                ls_resp = await client.get(
                    f"{self.base_url}/futures/data/topLongShortAccountRatio?symbol={symbol}&period=5m&limit=1"
                )
                if ls_resp.status_code == 200:
                    ls_data = ls_resp.json()
                    if ls_data and isinstance(ls_data, list):
                        snapshot["ls_ratio"] = float(
                            ls_data[0].get("longShortRatio", 0)
                        )
            except Exception as e:
