...
def parse_episodes_args(
    ...
    if command == "episodes-cut":
        ...
        parser.add_argument(
            "--format", dest="fmt", choices=["csv", "jsonl"], default=None
        )
        ...
        parser.add_argument(
            "--synthetic",
            action="store_true",
            help="Generate synthetic ticks if path is missing.",
        )
    ...
    if command == "episodes-run":
        ...
        parser.add_argument(
            "--replay-speed", choices=["instant", "realtime"], default="instant"
        )
        parser.add_argument(
            "--runs-path",
            type=Path,
            default=Path("SupervisorAgent") / "runtime" / "runs",
        )
        parser.add_argument("--scenarios-path", type=Path, default=None)
...
def run_episodes_command(command: str, args: argparse.Namespace) -> int:
    if command == "episodes-cut":
        ...
        if args.synthetic and not ticks_path.exists():
            out_dir = args.out_dir or (
                Path("SupervisorAgent") / "runtime" / "episodes" / args.episode_set
            )
    ...
    if command == "episodes-report":
        out_dir = args.out_dir or (
            Path("SupervisorAgent") / "runtime" / "reports" / args.episode_set
        )
