...
class MockExchange:
    def __init__(
        ...
        self.latency_ms = latency_ms  # Simulated delay (not used in simple event loop yet)
    ...
    def execute_order(self, symbol: str, side: str, qty: float, type: str, price: float = None) -> str:
        """
    ...
    def _fill_immediate(self, symbol, side, qty, price, order_id):
        ...
        self.trades.append(Trade(timestamp=self.current_time, symbol=symbol, side=side, price=price, qty=qty, fee=fee))
