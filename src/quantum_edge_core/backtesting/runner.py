...
class BacktestRunner:
    def __init__(self, symbol: str, start_time: datetime, end_time: datetime, db_host: str = "http://localhost:9000"):
        self.symbol = symbol
    ...
    def _update_candle(self, price: float, volume: float, ts: float):
        ...
        if self.current_candle is None:
            self.current_candle = {"t": bucket, "o": price, "h": price, "l": price, "c": price, "v": volume}
    ...
    def _generate_mock_ml_signal(self, features: np.ndarray) -> EnsembleOutput:
        ...
        return EnsembleOutput(meta_edge=edge, direction=1 if edge > 0 else (-1 if edge < 0 else 0), components=comps)
    ...
    def run(self):
        ...
        for event in events:
            ...
            if etype == "liquidation":
                ...
                # Update Candle
                self._update_candle(float(event["price"]), float(event["quantity"]), ts_float)
            ...
            cmd = decision.action
            if cmd == "enter" or (
                cmd == "hold" and decision.direction != 0 and self.exchange.positions.get(self.symbol, 0) == 0
            ):
                ...
                if pos == 0:
                    ...
                    qty = self.order_manager.calculate_entry_size(self.symbol, price, equity, self.policy)

                    if qty > 0:
                        # Place Market for simplicity in backtest
                        # (MockExchange supports Limit if we want to simulate SmartExecution)
                        self.exchange.execute_order(self.symbol, side, qty, "MARKET", price)

            elif cmd == "exit":
                pos = self.exchange.positions.get(self.symbol, 0)
                if pos != 0:
                    side = "SELL" if pos > 0 else "BUY"
                    self.exchange.execute_order(self.symbol, side, abs(pos), "MARKET", float(event["price"]))
