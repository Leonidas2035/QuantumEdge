...
def load_llm_supervisor_config(path: Path) -> LlmSupervisorConfig:
    ...
    if not path.exists():
        ...
        cb_default = CircuitBreakerConfig(3, 300, 120)
    ...
    circuit_breaker = CircuitBreakerConfig(
        ...
        open_sec=int(cb.get("open_sec", 120)),
    )
...
def load_snapshot_scheduler_config(path: Path) -> SnapshotSchedulerConfig:
    """Load snapshot scheduler configuration from hermes.yaml."""
...
def load_tsdb_config(path: Path) -> TsdbConfig:
    ...
    if not path.exists():
        return TsdbConfig(
            ...
            ingest_state_path="/home/korben/.hermes/runtime/ingest_state.json",
    ...
    return TsdbConfig(
        ...
        ingest_state_path=str(
            ingest.get("state_path", "/home/korben/.hermes/runtime/ingest_state.json")
        ),
