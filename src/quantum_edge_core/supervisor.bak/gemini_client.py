"""
/home/korben/.hermes/hermes/gemini_client.py
...
class AsyncCircuitBreaker:
        ...
        if self.state == CircuitState.OPEN:
            if now - self.last_failure_time > self.recovery_timeout:
                ...
                raise CircuitBreakerOpenException(
                    f"Circuit OPEN. Retry in {self.recovery_timeout - (now - self.last_failure_time):.1f}s"
                )
    ...
    def record_failure(self):
        ...
        if self.failures >= self.threshold:
            ...
            self.logger.warning(
                "Circuit OPEN - Threshold reached", failures=self.failures
            )
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code != 200:
                    self.logger.error(
                        "API Error", status=response.status_code, body=response.text
                    )
