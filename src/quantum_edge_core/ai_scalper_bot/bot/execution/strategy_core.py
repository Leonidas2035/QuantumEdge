...
class AdaptiveGridStrategy:
    ...
    def decide(
        self, market: MarketState, features: FeatureVector, atr: float, position: PositionManager
        ...
        if self.state == BotState.IDLE:
            ...
            if features.ofi > self.config.get("ofi_entry_threshold", 0.1):
                ...
                self.last_buy_price = current_price  # Temporarily set, confirmed on fill
                return TradeAction(
                    action_type="BUY",
            ...
            # A. Take Profit Check
            tp_price = position.avg_price * (1 + self.config.get("take_profit_pct", 0.01))
            if current_price >= tp_price:
                return TradeAction(
                    action_type="SELL", price=market.best_bid, qty=position.total_qty, reason="Take Profit Hit"
                )
            ...
            if current_price <= (self.last_buy_price - gap):
                ...
                return TradeAction(
                    action_type="BUY", price=market.best_ask, qty=dca_qty, reason=f"DCA Step (ATR Gap: {gap:.2f})"
                )
