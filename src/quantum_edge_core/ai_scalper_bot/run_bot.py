...
class BotEngine:
    def __init__(self):
        ...
        self.reporter = SupervisorReporter(pub_endpoint=f"tcp://*:{self.config.supervisor_port}")
        ...
        self.features = FeatureEngine(
            vpin_bucket_vol=10.0,  # Could also move to config
            vpin_window=50,
        )
        self.volatility = OnlineVolatility(alpha=self.config.strategy_config.get("atr_alpha", 0.01))
        self.strategy = AdaptiveGridStrategy(self.config.strategy_config)

        self.running = False

    async def run(self):
        logger.info(f">>> Bot Engine STARTING Main Loop... Target: {self.config.symbol}")
        self.running = True
            ...
            while self.running:
                ...
                if tick:
                    ...
                    if market_state:  # Ensure we have initial state
                        ...
                        action = self.strategy.decide(market_state, feat_vec, atr_val, self.position)
                        ...
                        if action:
                            ...
                            self.position.simulate_fill(action.price, action.qty, action.action_type)
                ...
                if now - last_heartbeat >= 1.0:
                    await self.reporter.send_heartbeat(
                        self.strategy.state, self.position.state.unrealized_pnl, self.position.total_qty
                    )
                    last_heartbeat = now
