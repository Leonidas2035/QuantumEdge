...
from quantum_edge_core.ai_scalper_bot.bot.execution.strategy_core import AdaptiveGridStrategy, BotState, TradeAction
...
def create_state(price):
    return MarketState(timestamp=1000, last_price=price, best_bid=price, best_ask=price, best_bid_qty=1, best_ask_qty=1)
...
def test_strategy_entry_ofi():
    config = {"ofi_entry_threshold": 5.0, "base_order_size_q": 0.1, "hedge_trigger_dd": 0.1}
...
def test_strategy_dca_atr():
    ...
    config = {"grid_step_atr_mult": 2.0, "base_order_size_q": 0.1, "hedge_trigger_dd": 0.5}
    strat = AdaptiveGridStrategy(config)
    pm = PositionManager()
