...
class DDNEngine:
    ...
    def evaluate(self, ctx: DDNContext, now_ms: Optional[int] = None) -> DDNDecision:
        ...
        if self._is_in_cooldown(now_ms) and intent.action not in {
            "PANIC_LOCK",
            "PAUSE",
        }:
            return DDNDecision(
        ...
        if (
            ctx.position.margin_usage is not None
            and ctx.position.margin_usage > self._cfg.max_margin_usage
        ):
        ...
        if (
            ctx.market.mark_price
            and adjusted_qty * ctx.market.mark_price < self._cfg.min_step_notional_usd
        ):
    ...
    def _panic_decision(self, ctx: DDNContext, now_ms: int) -> DDNDecision:
        ...
        plan = OrderPlan(
            ...
            limit_price=None,
            time_in_force=None,
            expected_cost_bps=self._estimate_cost_bps(
                DDNIntent(action="PANIC_LOCK"), ctx.market.funding_rate
            ),
        )
        self._record_action(now_ms)
        return DDNDecision(
    ...
    def _accept_profile_change(self, ctx: DDNContext) -> DDNDecision:
        target = (
            ctx.intent.target if ctx.intent.target is not None else ctx.profile.target
        )
        band_low = (
            ctx.intent.band_low
            if ctx.intent.band_low is not None
            else ctx.profile.band_low
        )
        band_high = (
            ctx.intent.band_high
            if ctx.intent.band_high is not None
            else ctx.profile.band_high
        )
    ...
    def _choose_qty(
        self, intent: DDNIntent, step_qty: float, mark_price: Optional[float]
    ) -> Optional[float]:
    ...
    def _clamp_to_band(
        self, ctx: DDNContext, qty: float, reasons: List[str]
    ) -> Optional[float]:
        delta = ctx.position.net_delta()
        ...
        if target + band_low <= next_delta <= target + band_high:
            return qty
        allowed_qty = _max_qty_within_band(
            delta, action, target + band_low, target + band_high
        )
    ...
    def _order_plan_for_intent(
        self, action: str, qty: float, cost_bps: float
    ) -> Optional[OrderPlan]:
    ...
    def _estimate_cost_bps(
        self, intent: DDNIntent, funding_rate: Optional[float]
    ) -> float:
        order_type = "MARKET" if intent.action == "PANIC_LOCK" else "LIMIT"
        fee = (
            self._cfg.taker_fee_bps
            if order_type == "MARKET"
            else self._cfg.maker_fee_bps
        )
        slippage = (
            self._cfg.expected_slippage_bps_market if order_type == "MARKET" else 0.0
        )
    ...
    def _reject(
        self, now_ms: int, reasons: List[str], cost_bps: float = 0.0
    ) -> DDNDecision:
...
def _clamp_band(
    band_low: float, band_high: float, max_abs: float
) -> tuple[float, float]:
...
def _max_qty_within_band(
    delta: float, action: str, band_low: float, band_high: float
) -> Optional[float]:
    if action == "ADD_LONG":
...
def _is_stale(
    market_lag_ms: Optional[int], account_lag_ms: Optional[int], limit_ms: int
) -> bool:
