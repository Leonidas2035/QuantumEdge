"""MarketDataHub subscriber for quantum_edge_core.lock_bot."""
...
class HubSubscriber:
    def __init__(
        ...
        self._queue: asyncio.Queue[Tuple[str, LockbotMarketEvent]] = asyncio.Queue()
    ...
    async def events(self) -> AsyncIterator[Tuple[str, LockbotMarketEvent]]:
        while True:
    ...
    async def _reader_loop(self) -> None:
        while not self._stop.is_set():
            try:
                topic, payload = await self._socket.recv_multipart()  # type: ignore[union-attr]
            ...
            try:
                ...
                continue
            self._queue.put_nowait((topic.decode("utf-8", errors="ignore"), event))
