...
from quantum_edge_core.strategies.scalper_v1.bot.engine.regime_policy import RegimePolicy
...
from quantum_edge_core.strategies.scalper_v1.bot.trading.trade_stats import TradeStats
...
class DecisionEngine:
    ...
    def __init__(self):
        ...
        self.max_daily_loss_abs = session_cfg.get(
            "max_daily_loss_abs", risk_cfg.get("max_daily_loss")
        )
        self.max_drawdown_abs = session_cfg.get(
            "max_drawdown_abs", risk_cfg.get("max_drawdown_abs")
        )
        self.max_trades_per_hour = session_cfg.get(
            "max_trades_per_hour", self.over_cfg.get("max_trades_per_hour", 60)
        )
    ...
    def _horizon_agreement(
        self, outputs: Dict[int, SignalOutput], reasons: List[str]
    ) -> (str, List[int]):
        ...
        if anchor in outputs:
            ...
            if (
                primary_dir == DecisionDirection.LONG
                and anchor_sig.p_down >= strong_anchor
            ):
                reasons.append("ANCHOR_OPPOSE")
                return DecisionDirection.NONE, used + [anchor]
            if (
                primary_dir == DecisionDirection.SHORT
                and anchor_sig.p_up >= strong_anchor
            ):
                reasons.append("ANCHOR_OPPOSE")
                return DecisionDirection.NONE, used + [anchor]
            used.append(anchor)
    ...
    def _confidence_gate(
        self, direction: str, ensemble: EnsembleOutput, reasons: List[str]
    ) -> bool:
        ...
        if direction == DecisionDirection.LONG:
            if (
                ensemble.meta_edge < self.min_edge
                or (0.5 + ensemble.meta_edge) < min_conf_long
            ):
            ...
            if (
                ensemble.meta_edge > -self.min_edge
                or (0.5 - ensemble.meta_edge) < min_conf_short
            ):
                reasons.append("LOW_CONFIDENCE")
                return False
        return True
    ...
    def _check_loss_streak(self, symbol: str, reasons: List[str]) -> bool:
        ...
        if self.cooldown_until.get(symbol, 0) > now:
            ...
            return False
        losses = stats.loss_streak(
            cfg.get("window_trades", 10), cfg.get("max_losses", 3)
        )
    ...
    def decide(
        ...
        if direction == DecisionDirection.NONE:
            return Decision(
                action=DecisionAction.NO_TRADE, reasons=reasons, horizons_used=used_hz
            )
        ...
        if position == 0:
            if not self._check_loss_streak(symbol, reasons):
                self.last_risk_state[symbol] = (
                    reasons[-1] if reasons else "LOSS_STREAK_COOLDOWN"
                )
                return Decision(
            ...
            if not self._check_overtrading(symbol, reasons):
                self.last_risk_state[symbol] = (
                    reasons[-1] if reasons else "OVERTRADING_LIMIT"
                )
                return Decision(
                    action=DecisionAction.NO_TRADE,
                    reasons=reasons,
        ...
        allow, min_conf_override, style = self.regime_policy.allow(
            regime_name, direction, self.min_confidence
        )
        trade_style = style
        if not allow:
        ...
        horizon_details = {
            h: HorizonDecision(
                ...
                direction=self._direction_from_signal(
                    sig, self.thresholds.get("min_conf_primary", 0.52)
                ),
        ...
        return Decision(
            ...
            direction=direction,
            confidence=(
                0.5 + ensemble.meta_edge
                if direction == DecisionDirection.LONG
                else 0.5 - ensemble.meta_edge
            ),
