...
class PolicyClient:
    ...
    def __init__(self, zmq_url: str | None = None):
        # Dynamically read port 5558 from config if not provided, default to 5558
        if zmq_url is None:
            # Attempt to load from centralized config if available
            try:
                from bot.core.config_loader import config

                policy_port = config.get("supervisor", {}).get("policy_port", 5558)
                self.zmq_url = f"tcp://127.0.0.1:{policy_port}"
            except ImportError:
                self.zmq_url = "tcp://127.0.0.1:5558"
        else:
