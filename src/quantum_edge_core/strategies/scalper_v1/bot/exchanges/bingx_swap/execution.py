...
class ExchangeInfoCache:
    ...
    def get(self, symbol: str) -> SymbolFilters:
        ...
        step_size = (
            size if size > 0 else (10 ** (-qty_precision) if qty_precision > 0 else 0.0)
        )
...
class BingXExecution:
    def __init__(
        self, client: BingXClient, info_cache: Optional[ExchangeInfoCache] = None
    ) -> None:
        self.client = client
    ...
    def place_order(self, req: OrderRequest) -> OrderResult:
        ...
        qty = (
            round_qty_to_step(req.qty, filters.step_size)
            if filters.step_size
            else req.qty
        )
        if filters.min_qty and qty < filters.min_qty:
            raise ValueError(
                f"Quantity {qty} below minQty {filters.min_qty} for {symbol}"
            )
        price = req.price
        ...
        if filters.min_notional and price is not None:
            ...
            if notional < filters.min_notional:
                raise ValueError(
                    f"Order notional {notional} below minNotional {filters.min_notional} for {symbol}"
                )
        ...
        filled_qty = float(
            order_data.get("executedQty") or order_data.get("filledQty") or 0.0
        )
    ...
    def get_positions(self, symbol: Optional[str] = None) -> List[Position]:
        ...
        for entry in items:
            ...
            qty = float(
                ...
                or entry.get("positionQty")
                or entry.get("positionAmount")
                or 0.0
            )
            ...
            results.append(
                Position(
                    ...
                    qty=qty,
                    entry_price=float(
                        entry.get("avgPrice") or entry.get("entryPrice") or 0.0
                    ),
    ...
    def get_balances(self) -> List[Balance]:
        ...
        for entry in items:
            ...
            results.append(
                Balance(asset=asset, available=available, total=total, raw=entry)
            )
        return results
