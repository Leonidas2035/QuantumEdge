...
class SmartExecutor:
    ...
    async def _execute_market(self, symbol: str, side: str, qty: float, reduce_only: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.client, "futures_create_order"):
                return await self.client.futures_create_order(
                    symbol=symbol, side=side, type="MARKET", quantity=qty, reduceOnly=reduce_only
                )
            else:
                return await self.client.create_order(symbol=symbol, side=side, type="MARKET", quantity=qty)
    ...
    async def _execute_limit_chase(
        ...
        while attempts < self.max_chase_attempts and remaining_qty > 0:
            ...
            logger.info(f"Chase {attempts}/{self.max_chase_attempts}: Limit {side} {symbol} {remaining_qty} @ {price}")
            ...
            try:
                if hasattr(self.client, "futures_create_order"):
                    ...
                    order = await self.client.create_order(
                        symbol=symbol, side=side, type="LIMIT", quantity=remaining_qty, price=price, timeInForce="GTC"
                    )
            ...
            remaining_qty = qty - executed  # Update remaining from total context if strictly tracking,
        ...
        if remaining_qty > 0:
            if urgency == "LOW":
                ...
                return await self._execute_market(symbol, side, remaining_qty, reduce_only)
    ...
    async def _get_bbo_price(self, symbol: str, side: str) -> Optional[float]:
        try:
            ...
            if hasattr(self.client, "futures_symbol_ticker"):  # Not ideal, looking for book ticker
