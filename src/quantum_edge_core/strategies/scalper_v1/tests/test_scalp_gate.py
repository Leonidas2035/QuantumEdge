...
def test_scalp_gate_spread_block():
    ...
    cfg = {"min_prob_up": 0.55, "min_edge": 0.0, "max_spread_bps": 1.0, "min_orderbook_depth_usd": 100.0}
    mode = ScalpExecutionMode(cfg, guard, order_policy=None, logger=None)
    decision = Decision(action=DecisionAction.ENTER, direction=DecisionDirection.LONG, size=1.0)
    signal = DummySignal(p_up=0.8, p_down=0.2, edge=0.1)
    last_event = {"b": 99.0, "a": 101.0, "q": 1.0, "depth": 1000.0}
    gate = mode.evaluate_entry(decision, price=100.0, symbol="BTCUSDT", signal=signal, last_event=last_event)
    assert not gate["ok"]
