...
def test_and_gate_passes():
    agg = MultiHorizonAggregator(
        policy="and_gate", thresholds={1: 0.6, 5: 0.6, 30: 0.6}
    )
    gate = agg.evaluate(
        _outputs(), DecisionDirection.LONG, now_ms=1, last_trade_ms=None
    )
    assert gate.allow
...
def test_and_gate_blocks():
    agg = MultiHorizonAggregator(
        policy="and_gate", thresholds={1: 0.8, 5: 0.6, 30: 0.6}
    )
    gate = agg.evaluate(
        _outputs(), DecisionDirection.LONG, now_ms=1, last_trade_ms=None
    )
...
def test_two_stage_blocks_on_h1():
    ...
    agg = MultiHorizonAggregator(
        policy="two_stage", thresholds={1: 0.6, 5: 0.6, 30: 0.6}
    )
