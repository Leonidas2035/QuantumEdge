...
def fetch_binance_data():
    ...
    try:
        ...
        depth_resp = requests.get(
            f"{BASE_URL}/v1/depth?symbol={SYMBOL}&limit=100"
        ).json()
        ...
        snapshot["biggest_bid_wall"] = {
            "price": float(biggest_bid[0]),
            "vol": float(biggest_bid[1]),
        }
        snapshot["biggest_ask_wall"] = {
            "price": float(biggest_ask[0]),
            "vol": float(biggest_ask[1]),
        }

        # 3. Свічки (Klines) - Останні 4H та 1H свічки
        kline_4h = requests.get(
            f"{BASE_URL}/v1/klines?symbol={SYMBOL}&interval=4h&limit=2"
        ).json()
        if len(kline_4h) > 1:
            last_closed_4h = kline_4h[-2]  # Беремо останню ЗАКРИТУ свічку
            snapshot["4H_trend"] = (
                "UP" if float(last_closed_4h[4]) > float(last_closed_4h[1]) else "DOWN"
            )

        # 4. Ф'ючерсні метрики (Дуже важливо для LLM аналізу!)
        # Funding Rate (Хто кому платить - лонги чи шорти)
        funding_resp = requests.get(
            f"{BASE_URL}/v1/premiumIndex?symbol={SYMBOL}"
        ).json()
