...
def find_repo_root(start: Optional[Path] = None) -> Path:
    ...
    for parent in [candidate] + list(candidate.parents):
        ...
        if (parent / "QuantumEdge.py").exists():
            return parent.resolve()
        if (parent / "quantum-edge-core" / "config" / "quantumedge.yaml").exists():
            return parent.resolve()
...
def get_paths() -> Dict[str, Path]:
    ...
    defaults: Dict[str, Path] = {
        "qe_root": qe_root,
        "config_dir": qe_root / "quantum-edge-core" / "config",
        "runtime_dir": qe_root / "runtime",
        "logs_dir": qe_root / "logs",
        "data_dir": qe_root / "data",
        "artifacts_dir": qe_root / "artifacts",
        "bot_dir": qe_root / "quantum-edge-core" / "strategies" / "scalper_v1",
        "supervisor_dir": qe_root / "quantum-edge-core" / "supervisor",
        "meta_agent_dir": qe_root / "quantum-edge-infra" / "automation" / "meta_agent",
        "bot_config_dir": qe_root / "quantum-edge-core" / "config",
        "supervisor_config_dir": qe_root / "quantum-edge-core" / "config",
        "meta_agent_config_dir": qe_root / "quantum-edge-core" / "config",
    }
