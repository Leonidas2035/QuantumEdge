...
from LockBotBTC.lockbot_btc.replay import load_dataset, load_ddn_config, load_policy_config, run_replay
from LockBotBTC.lockbot_btc.replay.scenarios import SCENARIO_NAMES, ScenarioConfig, generate_scenario
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--duration-s", type=int, default=7200, help="Scenario duration (seconds).")
    ...
    run.add_argument("--config", type=Path, default=Path("SupervisorAgent/configs/lockbot_btc_policy.yaml"))
    run.add_argument("--bot-config", type=Path, default=Path("LockBotBTC/config/lockbot_btc.yaml"))
    run.add_argument("--ddn-config", type=Path)
    run.add_argument("--account-topics", help="Comma-separated account topics (optional).")
    run.add_argument("--execution-enabled", action="store_true", help="Allow EXEC_STEP intents in replay.")
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    if args.command == "run":
        if args.scenario:
            ...
            meta = {"scenario": args.scenario, "duration_s": args.duration_s, "start_ts_ms": args.start_ts_ms}
        else:
            dataset_path = Path(args.dataset)
            events = load_dataset(dataset_path, time_min=args.time_min, time_max=args.time_max)
        ...
        ddn_cfg = load_ddn_config(args.ddn_config, base=bot_cfg.ddn) if args.ddn_config else None
