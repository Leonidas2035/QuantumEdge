...
def is_whitelisted_file(file_path):
    ...
    # Special handle for specific filenames if needed (e.g. .env.template
    # has a leading dot but is checked as extension by suffix usually? No,
    # suffix is .template). But user listed .env.template explicitly.
...
def generate_tree(start_path):
    ...
    def walk(current_path, prefix=""):
        ...
        for item in items:
            p = current_path / item
            if p.is_dir() and not should_exclude_dir(item, current_path):
                dirs.append(item)
