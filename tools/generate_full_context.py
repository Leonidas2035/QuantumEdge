...
WHITELIST_EXTENSIONS = {
    ...
    '.py', '.sh', '.js',
    # Config
    '.yaml', '.json', '.toml', '.ini', '.env.template',
    # Docs
    '.md', '.txt'
}
...
BLACKLIST_DIR_NAMES = {
    'venv', '.git', '.vscode', '__pycache__', 'node_modules', 'site-packages',
    'logs', 'data', 'db', 'artifacts', 'run'
}
...
def is_whitelisted_file(file_path):
    ...
    # Special handle for specific filenames if needed (e.g. .env.template has a leading dot but is checked as extension by suffix usually? No, suffix is .template)
