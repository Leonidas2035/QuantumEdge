...
TABLE_GROUPS = {
    ...
    "l2_telemetry": ["signals", "orders", "fills", "positions", "equity", "risk_events"],
}
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Purge QuestDB partitions based on retention policy.")
    parser.add_argument("--apply", action="store_true", help="Apply retention (otherwise dry-run).")
    ...
    retention = {
        ...
        "l2_telemetry": int(args.retention_l2 or retention_cfg.get("l2_telemetry", 180)),
    }
    ...
    for group, tables in TABLE_GROUPS.items():
        ...
        for table in tables:
            ...
            print(f"[apply] {table}: dropped partitions older than {days} days (as of {now_iso})")
