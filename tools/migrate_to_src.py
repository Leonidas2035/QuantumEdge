...
def safe_move_contents(src: Path, dest: Path):
    ...
    if not src.exists():
        logger.warning(f"SKIPPING: Source directory not found: {src.relative_to(PROJECT_ROOT)}")
        return
    ...
    for item in src.iterdir():
        ...
        if dest_path.exists():
            logger.warning(f"SKIPPING: Destination exists: {dest_path.relative_to(PROJECT_ROOT)}")
            continue

        try:
            shutil.move(str(item), str(dest_path))
            logger.info(f"MOVED: {item.relative_to(PROJECT_ROOT)} -> {dest_path.relative_to(PROJECT_ROOT)}")
...
def smart_move_file(filename: str, potential_sources: list[Path], dest: Path):
    ...
    for src in potential_sources:
        ...
        if src_file.exists():
            if dest.exists():
                logger.warning(f"SKIPPING: Destination exists for {filename}: {dest.relative_to(PROJECT_ROOT)}")
            ...
            try:
                ...
                logger.info(f"MOVED: {src_file.relative_to(PROJECT_ROOT)} -> {dest.relative_to(PROJECT_ROOT)}")
                found = True
                break
            except Exception as e:
    ...
    if not found:
        ...
        logger.info(f"SEARCHING: {filename} not found in expected paths, searching project...")
        for path in PROJECT_ROOT.rglob(filename):
            ...
            try:
                ...
                logger.info(f"MOVED: {path.relative_to(PROJECT_ROOT)} -> {dest.relative_to(PROJECT_ROOT)}")
                break
            except Exception as e:
...
def main():
    ...
    safe_move_contents(PROJECT_ROOT / "quantum-edge-core", src_dir / "quantum_edge_core")
    safe_move_contents(PROJECT_ROOT / "quantum-edge-ml", src_dir / "quantum_edge_ml")
    safe_move_contents(PROJECT_ROOT / "quantum-edge-infra", src_dir / "quantum_edge_infra")

    # 3. Smart File Search & Move
    logger.info("--- Moving Entry Points ---")

    # Move QuantumEdge.py -> scripts/run_orchestrator.py
    smart_move_file("QuantumEdge.py", [PROJECT_ROOT], scripts_dir / "run_orchestrator.py")
