...
def safe_move_contents(src: Path, dest: Path):
    ...
    for item in src.iterdir():
        ...
        try:
            ...
            logger.info(
                f"MOVED: {item.relative_to(PROJECT_ROOT)} -> {dest_path.relative_to(PROJECT_ROOT)}"
            )
...
def smart_move_file(filename: str, potential_sources: list[Path], dest: Path):
    ...
    for src in potential_sources:
        ...
        if src_file.exists():
            if dest.exists():
                logger.warning(
                    f"SKIPPING: Destination exists for {filename}: {dest.relative_to(PROJECT_ROOT)}"
                )
            ...
            try:
                ...
                logger.info(
                    f"MOVED: {src_file.relative_to(PROJECT_ROOT)} -> {dest.relative_to(PROJECT_ROOT)}"
                )
                found = True
                break
            except Exception as e:
    ...
    if not found:
        ...
        for path in PROJECT_ROOT.rglob(filename):
            ...
            try:
                ...
                logger.info(
                    f"MOVED: {path.relative_to(PROJECT_ROOT)} -> {dest.relative_to(PROJECT_ROOT)}"
                )
                break
            except Exception as e:
