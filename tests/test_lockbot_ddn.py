...
def test_margin_guard_blocks_orders(service):
    ...
    command = {
        ...
        "payload": {"cmd": "EXEC_STEP", "action": "ADD_LONG", "qty_hint": 0.1},
    }
...
def test_vwap_fading_micro_averaging(service):
    ...
    command = {
        ...
        "payload": {"cmd": "EXEC_STEP", "action": "ADD_SHORT", "qty_hint": 0.5},
    }

    ack = service.process_command(command)
    assert ack.payload["status"] in [
        "ACCEPTED",
        "REJECTED",
    ]  # May reject due to clamp if qty is too big
