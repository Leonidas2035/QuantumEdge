    ...
    def update_event(self, event: LockbotMarketEvent | dict) -> None:
        with self._lock:
            ...
            if event_type == "mark_price_1s":
                ...
                self.avwap, self.avwap_anchor, self.avwap_anchors = _select_avwap(payload)
                ...
                self.liq_summary = _summarize_heatmap(payload, self.mark_price, ts_event)
...
class LockbotHubSubscriber:
    def __init__(self, endpoint: str, topics: Iterable[str], cache: MarketDataCache, rcv_hwm: int = 1000) -> None:
...
def _select_avwap(payload: dict) -> tuple[Optional[float], Optional[str], dict[str, float]]:
...
def _summarize_heatmap(payload: dict, mark_price: Optional[float], ts_event: int) -> LiqHeatmapSummary:
    intensity_above = _to_float(payload.get("intensity_above")) or 0.0
    intensity_below = _to_float(payload.get("intensity_below")) or 0.0
    if intensity_above or intensity_below or not isinstance(payload.get("levels"), list):
        return LiqHeatmapSummary(intensity_above=intensity_above, intensity_below=intensity_below, last_ts=ts_event)
    if mark_price is None:
        return LiqHeatmapSummary(intensity_above=0.0, intensity_below=0.0, last_ts=ts_event)
    above = 0.0
    below = 0.0
