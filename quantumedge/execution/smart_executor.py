...
class SmartMakerExecutor:
        ...
        *,
        ...
        if not self._market_quality_ok(book):
            return self._finalize(
                session, start_ms, ExecutionStatus.ABORTED, "market_quality"
            )
        if not self._slippage_ok(book, request.signal_price):
            return self._finalize(
                session, start_ms, ExecutionStatus.ABORTED, "slippage_guard"
            )

        maker_price = self._maker_price(book, request.side)
        if maker_price is None:
            return self._finalize(
                session, start_ms, ExecutionStatus.ABORTED, "no_price"
            )

        session.last_mid = self._mid_price(book)
        if self._below_minimums(session.remaining_qty, maker_price):
            return self._finalize(
                session, start_ms, ExecutionStatus.DONE, "min_remaining"
            )

        ack = await self._place_maker(request, session, maker_price)
        if not ack:
            return self._finalize(
                session, start_ms, ExecutionStatus.FAILED, "place_failed"
            )
        ...
        while session.state not in {
            ...
            now_ms = self._now_ms()
            if (
                self._cfg.max_lifetime_ms
                and now_ms - start_ms > self._cfg.max_lifetime_ms
            ):
                await self._cancel_open(request, session, reason="lifetime_guard")
                return self._finalize(
                    session, start_ms, ExecutionStatus.ABORTED, "timeout"
                )
            ...
            if session.state == OrderState.PARTIAL and self._below_minimums(
                session.remaining_qty, session.price
            ):
            ...
            if book:
                ...
                if self._should_reprice(session, book) and self._can_replace(
                    now_ms, session
                ):
            ...
            maker_timeout = (
                now_ms - session.maker_start_ts >= self._cfg.maker_timeout_ms
            )
            ...
            if (maker_timeout or reprices_exhausted) and session.remaining_qty > 0:
                if self._cfg.order_policy == OrderPolicy.MAKER_ONLY:
                    ...
                    return self._finalize(
                        session, start_ms, ExecutionStatus.ABORTED, "maker_only"
                    )
                ...
                return self._finalize(
                    session, start_ms, ExecutionStatus.ABORTED, "fallback_failed"
                )
    ...
    def _emit_transition(
        self, session: OrderSession, transition: str, **extra: object
    ) -> None:
        ...
        payload = {
            ...
            "avg_fill_price": (
                str(session.avg_fill_price) if session.avg_fill_price else None
            ),
    ...
    def _slippage_ok(self, book: BookState, signal_price: Decimal) -> bool:
        ...
        if not mid:
            return False
        if (
            self._cfg.max_slippage_abs
            and abs(mid - signal_price) > self._cfg.max_slippage_abs
        ):
            return False
    ...
    def _market_quality_ok(self, book: BookState) -> bool:
        ...
        if (
            self._cfg.min_top_depth_qty is not None
            and top_qty_sum < self._cfg.min_top_depth_qty
        ):
            return False
    ...
    def _below_minimums(self, qty: Decimal, price: Optional[Decimal]) -> bool:
        if (
            self._cfg.min_remaining_qty is not None
            and qty < self._cfg.min_remaining_qty
        ):
            return True
    ...
    async def _place_maker(
        self, request: OrderRequest, session: OrderSession, price: Decimal
    ) -> Optional[OrderAck]:
    ...
    async def _place_aggressive(
        self, request: OrderRequest, session: OrderSession, book: BookState
    ) -> Optional[OrderAck]:
    ...
    async def _place_market(
        self, request: OrderRequest, session: OrderSession
    ) -> Optional[OrderAck]:
        placement = OrderPlacement(
            symbol=request.symbol,
            side=request.side,
    ...
    async def _cancel_open(
        self, request: OrderRequest, session: OrderSession, *, reason: str
    ) -> None:
    ...
    def _should_reprice(self, session: OrderSession, book: BookState) -> bool:
        ...
        if session.reprices >= self._cfg.max_reprices:
            return False
        if (
            session.last_reprice_ts
            and self._now_ms() - session.last_reprice_ts
            < self._cfg.min_reprice_interval_ms
        ):
            return False
    ...
    async def _apply_fallback(
        ...
        if self._cfg.fallback_policy == FallbackPolicy.NONE:
            ...
            return self._finalize(
                session, start_ms, ExecutionStatus.ABORTED, "fallback_none"
            )
        if self._cfg.fallback_policy == FallbackPolicy.AGGRESSIVE_LIMIT and book:
            ...
            if not ack:
                return None
            session = self._apply_ack(
                session, ack, self._aggressive_price(book, request.side)
            )
            return self._finalize(
                session, start_ms, ExecutionStatus.DONE, "fallback_aggressive"
            )
        if self._cfg.fallback_policy == FallbackPolicy.MARKET:
            ...
            return self._finalize(
                session, start_ms, ExecutionStatus.DONE, "fallback_market"
            )
        return None

    async def _drain_reports(
        self, reports: Optional[asyncio.Queue[ExecutionReport]], session: OrderSession
    ) -> None:
        if not reports:
            return
        while True:
    ...
    def _apply_report(
        self, session: OrderSession, report: ExecutionReport
    ) -> OrderSession:
    ...
    def _apply_ack(
        self, session: OrderSession, ack: OrderAck, price: Optional[Decimal]
    ) -> OrderSession:
        ...
        if ack.filled_qty:
            ...
            session.remaining_qty = max(
                session.remaining_qty - ack.filled_qty, Decimal("0")
            )
        if ack.avg_price:
    ...
    def _finalize(
        ...
        if session.price and session.last_mid and session.last_mid > 0:
            effective_spread = (
                (session.price - session.last_mid) / session.last_mid * Decimal("10000")
            )
