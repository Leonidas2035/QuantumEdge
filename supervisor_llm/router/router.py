...
from supervisor_llm.contracts.decision_v1 import DecisionV1, decode_decision, fallback_decision
from supervisor_llm.decoding.enforce import enforce_decision
from supervisor_llm.router.budgets import BudgetConfig, TeacherBudgets
from supervisor_llm.router.cache import RouterCache
from supervisor_llm.router.circuit import CircuitBreaker, CircuitConfig
from supervisor_llm.router.distill import DistillConfig, DistillWriter
from supervisor_llm.router.policy import RouterPolicy
from supervisor_llm.router.redaction import RedactionResult, redact_prompt
    ...
    def route(
        ...
        def call_student():
            ...
            if self.student_circuit.is_open(time.time()):
                return None
            attempts += 1
            result = enforce_decision(prompt, self.student_backend, timeout_s=timeout_s)
            if result.ok:
        ...
        def call_teacher():
            ...
            if not self.budgets.can_use(now_utc, tokens_est):
                return None
            attempts += 1
            result = enforce_decision(prompt, self.teacher_backend, timeout_s=timeout_s)
