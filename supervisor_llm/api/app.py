...
from supervisor_llm.backends.openai_chat import OpenAIChatBackend
from supervisor_llm.backends.openai_responses import OpenAIResponsesBackend
from supervisor_llm.backends.trtllm_openai_compat import OpenAICompatBackend
...
from supervisor_llm.context.fetcher import build_fetcher_from_env
from supervisor_llm.context.formatter import ContextFormatter
from supervisor_llm.router.redaction import hash_prompt
from supervisor_llm.router.router import Router
...
def _get_backend():
    global _backend
    if _backend is None:
        backend_type = os.environ.get("SUPERVISOR_LLM_BACKEND", "openai_compat")
...
def decision(req: DecisionRequest) -> Response:
    try:
        backend = _get_backend()
        result = enforce_decision(req.prompt, backend, timeout_s=req.timeout_s)
...
def decision_routed(req: RoutedDecisionRequest) -> Response:
    try:
        router = _get_router()
        prompt = req.prompt
        ...
        result = router.route(prompt, timeout_s=req.timeout_s, hints=hints)
        payload = result.decision.to_compact_json()
        try:
