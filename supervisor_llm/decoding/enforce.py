...
from supervisor_llm.contracts.decision_v1 import ValidationError, decode_decision, fallback_decision
from supervisor_llm.decoding.repair_prompts import SYSTEM_PROMPT, make_repair_prompt, make_user_prompt
...
def enforce_decision(prompt: str, backend, *, timeout_s: float = 2.0, max_attempts: int = 2) -> EnforceResult:
    ...
    for attempt in range(max_attempts):
        attempts += 1
        try:
            raw = backend.generate(current_prompt, system_prompt=SYSTEM_PROMPT, timeout_s=timeout_s)
            last_raw = raw
