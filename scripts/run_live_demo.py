...
def main():
    ...
    try:
        ...
        t_feed = threading.Thread(target=stream_reader, args=(feed_proc.stdout, "[FEED]", YELLOW))
        t_feed.daemon = True
        t_feed.start()
        ...
        t_bot = threading.Thread(target=stream_reader, args=(bot_proc.stdout, "[BOT ]", GREEN))
        t_bot.daemon = True
        t_bot.start()
