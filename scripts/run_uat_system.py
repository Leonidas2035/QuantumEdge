...
def main():
    ...
    try:
        ...
        t_mock = threading.Thread(
            target=stream_reader, args=(mock_proc.stdout, "[MOCK]", YELLOW, stop_event, success_flags)
        )
        t_mock.daemon = True
        t_mock.start()
        ...
        t_bot = threading.Thread(
            target=stream_reader, args=(bot_proc.stdout, "[BOT ]", GREEN, stop_event, success_flags)
        )
        t_bot.daemon = True
        t_bot.start()
