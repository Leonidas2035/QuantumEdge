...
async def mock_market_feed(ctx):
    """Emulate MarketDataHub: sends ticks every 10ms."""
...
async def supervisor_listener(ctx):
    """Emulate Supervisor: listens if bot is alive."""
...
async def main():
    ...
    print("1. Starting Mock Market Feed...")
    print("2. Starting Mock Supervisor Listener...")
    print("3. NOW RUN THE BOT IN ANOTHER TERMINAL!")
    print("-" * 30)

    await asyncio.gather(mock_market_feed(ctx), supervisor_listener(ctx))
