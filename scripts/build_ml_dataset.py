...
def build_features():
    ...
    df_pivoted = df.pivot_table(index='timestamp', columns='side', values=['price', 'qty'], aggfunc='first').dropna()
    
    df_pivoted.columns = [f"{col[0]}_{str(col[1]).upper()}" for col in df_pivoted.columns]
    
    # КЛЮЧОВИЙ ФІКС: BUY/SELL замість BID/ASK
    df = df_pivoted.rename(columns={
        'price_BUY': 'best_bid',
        'price_SELL': 'best_ask',
