...
class GeminiNativeClient:
    ...
    def __init__(
        ...
        **_: Any,
        ...
        if api_key_clean in {"google-oauth", "adc"}:
            self.use_adc = True
        elif not api_key_clean:
            # Check if base_url is a Vertex AI url or if we can fall back to ADC
            if base_url and "aiplatform.googleapis.com" in base_url.lower():
                self.use_adc = True
            else:
                try:
                    import google.auth
                    self._adc_creds, self._adc_project = google.auth.default(
        ...
        if not self.use_adc and not api_key_clean:
            raise RuntimeError(
        ...
        if not self.use_adc and api_key_clean:
            try:
                import google.auth
                google.auth.default(scopes=["https://www.googleapis.com/auth/cloud-platform"])
                import logging
                logging.getLogger(__name__).warning(
                    "GOOGLE_API_KEY is active, but Google Application Default Credentials (ADC) are also available. "
                    "If your GCP project has a security policy disallowing API keys (resulting in HTTP 403 block errors), "
                    "run 'unset GOOGLE_API_KEY' and 'unset GEMINI_API_KEY' in your terminal before running hermes."
                )
            except Exception:
                pass

        self.api_key = api_key_clean or "adc"
        normalized_base = (base_url or "").strip().rstrip("/")
        
        if self.use_adc:
            # Auto-route to Vertex AI if base_url is empty or points to AI Studio (since AI Studio doesn't work with cloud-platform scopes)
            if not normalized_base or "generativelanguage.googleapis.com" in normalized_base:
                import os
                region = (
                ...
                if self._adc_project is None:
                    try:
                        import google.auth
                        _, self._adc_project = google.auth.default(
                            scopes=["https://www.googleapis.com/auth/cloud-platform"]
                        )
                    except Exception:
                        pass
                
                if not self._adc_project:
                    import agent.google_oauth as google_oauth
                    self._adc_project = google_oauth.resolve_project_id_from_env()

                if not self._adc_project:
                    raise RuntimeError(
                        "Application Default Credentials (ADC) are active, but no Google Cloud "
                        "project ID could be resolved. Please set GOOGLE_CLOUD_PROJECT or "
                        "HERMES_GEMINI_PROJECT_ID environment variable."
                    )
                normalized_base = f"https://{region}-aiplatform.googleapis.com/v1/projects/{self._adc_project}/locations/{region}/publishers/google"
                logger.info("ADC active: auto-routed Gemini native client to Vertex AI at %s", normalized_base)
        else:
            if not normalized_base:
                normalized_base = DEFAULT_GEMINI_BASE_URL
    ...
    def _headers(self) -> Dict[str, str]:
        ...
        if self.use_adc:
            ...
            if self._adc_project:
                headers["x-goog-user-project"] = self._adc_project
        else:
        ...
        **_: Any,
        ...
        if self.use_adc:
            ...
            if mapped_model:
                import logging
                logging.getLogger(__name__).warning(
                    f"Model '{model}' is not supported by Vertex AI. Auto-routing to '{mapped_model}'."
                )
                model = mapped_model
        ...
        if isinstance(extra_body, dict):
            ...
            if thinking_config and isinstance(thinking_config, dict):
                ...
                if not normalized_model.startswith(("gemini-3", "gemini-3.1")):
