...
def test_native_client_rejects_empty_api_key_with_actionable_message(monkeypatch):
    ...
    # Mock google.auth to simulate no ADC configured
    import google.auth
    monkeypatch.setattr(google.auth, "default", lambda *a, **k: (_ for _ in ()).throw(ValueError("No ADC")))
...
def test_native_client_with_adc_and_vertex_routing(monkeypatch):
    from agent.gemini_native_adapter import GeminiNativeClient, is_native_gemini_base_url
    
    assert is_native_gemini_base_url("https://us-central1-aiplatform.googleapis.com/v1/projects/cyberclaw-496012/locations/us-central1/publishers/google")
    ...
    import google.auth
    monkeypatch.setattr(google.auth, "default", lambda *a, **k: (DummyCreds(), "mock-project-123"))

    client = GeminiNativeClient(api_key="")
    assert client.use_adc is True
    assert "mock-project-123" in client.base_url
    assert "us-central1" in client.base_url
    
    headers = client._headers()
    assert headers["Authorization"] == "Bearer mock-token"
    assert headers["x-goog-user-project"] == "mock-project-123"
    assert "x-goog-api-key" not in headers


def test_native_client_strips_thinking_level_for_2_5(monkeypatch):
    from agent.gemini_native_adapter import GeminiNativeClient
    ...
    client.chat.completions.create(
        model="gemini-2.5-flash",
        messages=[{"role": "user", "content": "Hello"}],
        extra_body={"thinking_config": {"thinking_level": "medium", "include_thoughts": True}}
    )

    gen_config = recorded_json.get("generationConfig", {})
