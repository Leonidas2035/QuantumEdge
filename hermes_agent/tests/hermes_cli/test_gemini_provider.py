...
class TestGeminiCredentials:
    ...
    def test_resolve_with_adc(self, monkeypatch):
        ...
        with patch("google.auth.default", return_value=(DummyCreds(), "test-project-123")):
            creds = resolve_api_key_provider_credentials("gemini")
            assert creds["api_key"] == "adc"
            assert creds["source"] == "google-auth"
    ...
    def test_runtime_gemini_with_adc(self, monkeypatch):
        monkeypatch.delenv("GOOGLE_API_KEY", raising=False)
        monkeypatch.delenv("GEMINI_API_KEY", raising=False)
        ...
        with patch("google.auth.default", return_value=(DummyCreds(), "test-project-123")):
            ...
            result = resolve_runtime_provider(requested="gemini")
            assert result["provider"] == "gemini"
            assert result["api_key"] == "adc"
            assert result["source"] == "google-auth"
