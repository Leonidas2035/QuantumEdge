...
class TestLoadEnvComments:
    def test_load_env_strips_trailing_comments(self, tmp_path):
        env_file = tmp_path / ".env"
        env_file.write_text(
            "GEMINI_API_KEY=adc  # alias for GOOGLE_API_KEY\n"
            "QUOTED_KEY=\"val # comment\"\n"
            "ANOTHER_KEY=val2\t# tab comment\n"
        )
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            from hermes_cli.config import invalidate_env_cache, load_env
            invalidate_env_cache()
            env = load_env()
            assert env.get("GEMINI_API_KEY") == "adc"
            assert env.get("QUOTED_KEY") == "val # comment"
            assert env.get("ANOTHER_KEY") == "val2"
